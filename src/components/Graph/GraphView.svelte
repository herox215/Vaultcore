<script lang="ts">
  ...
  // to commit its response to `data`. Without this, a slow load racing a
  // fresh one could overwrite the newer result with the stale one.
  ...
  async function refetch(): Promise<void> {
    ...
    errorMessage = null;
    try {
      const result = await getLinkGraph();
